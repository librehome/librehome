...
function lpeg_qa()
	...
	local t, n = m.match(m.Ct(m.C(letter)^1) * m.Cc"t", "alo")
	...
	p = m.Ct(m.Cg(1, print) * m.Cg(1, 23.5) * m.Cg(1, assert))
	t = p:match('abcdefghij')
	assert(t[print] == 'a' and t[23.5] == 'b' and t[assert] == 'c')
	...
	local B = #letter * -m.B(letter) + -letter * m.B(letter)
	local x = m.Ct({ (B * m.Cp())^-1 * (1 * m.V(1) + m.P(true)) })
	...
	local t1 = {1,1,nil,nil,4,nil,3,nil,nil}
	...
	t = {}
	local function foo (p) t[#t + 1] = p; return p .. "x" end
	...
	local defs = {digits = m.R"09", letters = m.R"az", _=m.P"__"}
	...
	local function eq (_, _, a, b) return a == b end
	...
	local function addtag (s, i, t, tag) t.tag = tag; return i, t end
