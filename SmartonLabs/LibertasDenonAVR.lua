...
local ENUM_MS = {"UNKNOWN", "MOVIE", "MUSIC", "GAME", "DIRECT", "PURE DIRECT", "STEREO", "AUTO", "DOLBY PRO LOGIC", "DOLBY PL2 C", "DOLBY PL2 M", "DOLBY PL2 G", "DOLBY PL2X C", "DOLBY PL2X M", "DOLBY PL2X G", "DOLBY PL2Z H", "DOLBY SURROUND", "DOLBY ATMOS", "DOLBY DIGITAL", "DOLBY D EX", "DOLBY D+PL2X C", "DOLBY D+PL2X M", "DOLBY D+PL2Z H", "DOLBY D+DS", "DOLBY D+NEO:X C", "DOLBY D+NEO:X M", "DOLBY D+NEO:X G", "DOLBY D+NEURAL:X", "DTS SURROUND", "DTS ES DSCRT6.1", "DTS ES MTRX6.1", "DTS+PL2X C", "DTS+PL2X M", "DTS+PL2Z H", "DTS+DS", "DTS96/24", "DTS96 ES MTRX", "DTS+NEO:6", "DTS+NEO:X C", "DTS+NEO:X M", "DTS+NEO:X G", "DTS+NEURAL:X", "DTS ES MTRX+NEURAL:X", "DTS ES DSCRT+NEURAL:X", "MULTI CH IN", "M CH IN+DOLBY EX", "M CH IN+PL2X C", "M CH IN+PL2X M", "M CH IN+PL2Z H", "M CH IN+DS", "MULTI CH IN 7.1", "M CH IN+NEO:X C", "M CH IN+NEO:X M", "M CH IN+NEO:X G", "M CH IN+NEURAL:X", "DOLBY D+", "DOLBY D+ +EX", "DOLBY D+ +PL2X C", "DOLBY D+ +PL2X M", "DOLBY D+ +PL2Z H", "DOLBY D+ +DS", "DOLBY D+ +NEO:X C", "DOLBY D+ +NEO:X M", "DOLBY D+ +NEO:X G", "DOLBY D+ +NEURAL:X", "DOLBY HD", "DOLBY HD+EX", "DOLBY HD+PL2X C", "DOLBY HD+PL2X M", "DOLBY HD+PL2Z H", "DOLBY HD+DS", "DOLBY HD+NEO:X C", "DOLBY HD+NEO:X M", "DOLBY HD+NEO:X G", "DOLBY HD+NEURAL:X", "DTS HD", "DTS HD MSTR", "DTS HD+PL2X C", "DTS HD+PL2X M", "DTS HD+PL2Z H", "DTS HD+DS", "DTS HD+NEO:6", "DTS HD+NEO:X C", "DTS HD+NEO:X M", "DTS HD+NEO:X G", "DTS HD+NEURAL:X", "DTS:X", "DTS:X MSTR", "DTS EXPRESS", "DTS ES 8CH DSCRT", "MPEG2 AAC", "AAC+DOLBY EX", "AAC+PL2X C", "AAC+PL2X M", "AAC+PL2Z H", "AAC+DS", "AAC+NEO:X C", "AAC+NEO:X M", "AAC+NEO:X G", "AAC+NEURAL:X", "PL DSX", "PL2 C DSX", "PL2 M DSX", "PL2 G DSX", "PL2X C DSX", "PL2X M DSX", "PL2X G DSX", "AUDYSSEY DSX", "DTS NEO:6 C", "DTS NEO:6 M", "DTS NEO:X C", "DTS NEO:X M", "DTS NEO:X G", "NEURAL:X", "NEO:6 C DSX", "NEO:6 M DSX", "AURO3D", "AURO2DSURR", "MCH STEREO", "WIDE SCREEN", "SUPER STADIUM", "ROCK ARENA", "JAZZ CLUB", "CLASSIC CONCERT", "MONO MOVIE", "MATRIX", "VIDEO GAME", "VIRTUAL", "STEREO", "ALL ZONE STEREO", "7.1IN"}
...
local function DenonAVR(avr, mainZone, extraZones)
    ...
    local function onControlOnOff(event, cmd)
        ...
        elseif event.d == LibertasCommand.TOGGLE then
            local curValue = deviceValues:get(event.id)
            if curValue == nil then
                curValue = false
            end
    ...
    local function onControlEnum(event, cmd, values)
        ...
        local attrs = event.v
        local v = attrs[bit.bor(
            bit.lshift(LibertasClusterId.GEN_MULTISTATE_INPUT_BASIC, 16),
            LibertasAttrId.IOV_BASIC_PRESENT_VALUE
        )]
        if v ~= nil then
    ...
    Libre_SetOnNetReady(
        ...
        function(tag)
            ...
            enqueuePendingMessages(mainZone.mode, "MS?\r", 2, AckAction.FORCE, LibertasEventSource.REPORT)
            if #extraZones > 0 then
